$(function(){
		...
		$(".getcode").on('click',{"url":url,"data":{"data":"data"}},askData);		
	 	...
	 	//提交注册
	 	$(".registered").on('click',{"url":url,"data":{"data":"data"}},askSubmit);
	 	function askSubmit(data){
	 		...
	 		if(!sidecode){
			    $(".sidecode input").focus();
			    sidecode.fadeIn(100,function(){
			    	 sidecode.fadeOut(1000)
			    });
	 		...
	 		ajax.done(function(data){
	 			...
	 			$(".form_submit").find('span').text("修改成功");
	 		})
	 		ajax.fail(function(){
	 			console.log(22)
	 		})
